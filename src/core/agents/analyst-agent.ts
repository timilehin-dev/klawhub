...
export class AnalystAgent extends BaseAgent {
    ...
    switch (message.type) {
      case "request":
        if (message.payload.type === "analyze_data") {
          await this.analyzeData(message.payload.data, message.from, message.payload.query, message.correlationId);
  ...
  async executeTask(task: any): Promise<any> {
    return this.analyzeData(task.data, task.from || "system", task.query, task.correlationId);
  }

  private async analyzeData(data: any, requester: string, query?: string, correlationId?: string): Promise<void> {
    try {
  ...
  private generateAnalysisCode(data: any, query?: string): string {
    // Avoid direct string interpolation of query into code if possible.
    // For now, we'll wrap the data and use a safer access pattern.
...
# Custom analysis with basic sanitization
try:
    # Use a limited set of allowed pandas methods for the query if it's a simple string
    # This is still not 100% secure but better than arbitrary injection.
    # In a production environment, this should be generated by an LLM or restricted to a DSL.
    result = df.${escapedQuery}

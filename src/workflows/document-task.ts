...
import { generateDocument, generateOutline } from "@/core/agents/documentor";
import {
  postToThread,
  updateMessage,
  addReaction,
...
import { updateTask, trackSkillUsage } from "@/db";
...
export const documentWorkflow = inngest.createFunction(
  ...
  async ({ event, step }): Promise<void> => {
    ...
    try {
      ...
      // Error boundary — catch unhandled errors from any step
      ...
      try {
        ...
        console.error("[DOCUMENT] Error notification failed:", notifyError);
      }
    }
  }
);

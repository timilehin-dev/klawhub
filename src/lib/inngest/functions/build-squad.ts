...
import { writeCodeFromLearnings, fixCode } from "@/lib/agents/engineer";
import { testCode, persistLearning } from "@/lib/agents/qa";
    ...
    try {
      ...
      const approval = await step.run("wait-for-approval", async () => {
        ...
        const blocks = approvalBlocks(
          ...
          `*Language:* ${specResult.language}${depsSection}\n\n${specResult.spec}`,
          runId,
          "build_spec"
        );
